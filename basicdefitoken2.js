...
contract basicdefitoken is Context, ERC20, Ownable, basicdefitoken, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedMintCaller() {
        require(_msgSender() == owner() || _authorizedMintCaller[_msgSender()],"basicdefitoken: MINT_CALLER_NOT_AUTHORIZED");
        _;
    }

    constructor () public ERC20("basicdefitoken", "basicdefitoken") {}
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override(ERC20, IERC20) returns (bool) {
        ...
        _approve(sender, _msgSender(), allowance.sub(amount, "basicdefitoken: TRANSFER_AMOUNT_EXCEEDED"));
        return true;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "basicdefitoken: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "basicdefitoken: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "basicdefitoken: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "basicdefitoken: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedMintCaller external override {
        ...
        require(totalSupply() <= 10**26, "basicdefitoken: TOTAL_SUPPLY_EXCEEDED");
    }
